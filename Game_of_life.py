...
class Cell:
    def __init__(self, x, y, alive=0):
        ...
        self._X = x  # X, Y Point in map
        self._Y = y  # X, Y Point in map
        self._alive = alive
        self._nieghberhoods = []
        ...
        for xi in range(-1, 2):
            for yi in range(-1, 2):
                if (
                    self._X + xi < 0
                    or self._X + xi >= GAME_PLACE_LIMIT
                    or self._Y + yi < 0
                    or self._Y + yi >= GAME_PLACE_LIMIT
                    or (xi == 0 and yi == 0)
                ):
                    continue
                else:
...
class Generations:
    ...
    def start(self):
        ...
        while self._generations_iter < self.max_iter:
            ...
            self.world.update()  # 更新 world map
    ...
    def __init__(self):
        ...
        for y in range(GAME_PLACE_LIMIT):
            ...
            for x in range(GAME_PLACE_LIMIT):
                self.map[y].append(Cell(x, y, 0) if random.random() > 0.35 else Cell(x, y, 1))
